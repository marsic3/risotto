    ...
    <page-content>
      <b-container>
        <b-row no-gutters>
          ...
          <b-col lg="9">
            <!-- Menu Category -->
            <menu-category v-for="(category, categoryIndex) in categories" :id="category.name.replace(/\s/g, '')" :key="categoryIndex" :image="category.image" :name="category.name">
              <!-- Menu Item -->
              <menu-list-item v-for="(item, itemIndex) in itemsByCategoryId(category.id)" :key="itemIndex" :item="item" />
            </menu-category>
          </b-col>
        </b-row>
      </b-container>
